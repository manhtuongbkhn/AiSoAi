...
//	private static final String TAG_TEXTURE_ATTRIBUTE_HEIGHT = "height";
	...
	private ITexture parseTexture(final Attributes pAttributes) throws TexturePackParseException {
		...
		if(type.equals(TexturePackParser.TAG_TEXTURE_ATTRIBUTE_TYPE_VALUE_BITMAP)) {
			...
			try {
				texture = new PVRTexture(this.mTextureManager, PVRTexture.PVRTextureFormat.fromPixelFormat(pixelFormat), new SmartPVRTexturePixelBufferStrategy(DataConstants.BYTES_PER_MEGABYTE / 8), textureOptions) {
					@Override
			...
			try {
				texture = new PVRGZTexture(this.mTextureManager, PVRTexture.PVRTextureFormat.fromPixelFormat(pixelFormat), new SmartPVRTexturePixelBufferStrategy(DataConstants.BYTES_PER_MEGABYTE / 8), textureOptions) {
					@Override
					protected InputStream onGetInputStream() throws IOException {
						return TexturePackParser.this.onGetInputStream(file);
					}
			...
			try {
				texture = new PVRCCZTexture(this.mTextureManager, PVRTexture.PVRTextureFormat.fromPixelFormat(pixelFormat), new SmartPVRTexturePixelBufferStrategy(DataConstants.BYTES_PER_MEGABYTE / 8), textureOptions) {
					@Override
					protected InputStream onGetInputStream() throws IOException {
						return TexturePackParser.this.onGetInputStream(file);
					}
