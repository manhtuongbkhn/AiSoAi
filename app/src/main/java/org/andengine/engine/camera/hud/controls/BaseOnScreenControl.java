...
public abstract class BaseOnScreenControl extends HUD implements IOnSceneTouchListener {
	...
	protected void onUpdateControlKnob(final float pRelativeX, final float pRelativeY) {
		...
		final float x = controlBaseSceneCenterCoordinates[Constants.VERTEX_INDEX_X] - controlKnob.getWidth() * 0.5f + pRelativeX * controlBase.getWidthScaled();
		final float y = controlBaseSceneCenterCoordinates[Constants.VERTEX_INDEX_Y] - controlKnob.getHeight() * 0.5f + pRelativeY * controlBase.getHeightScaled();
